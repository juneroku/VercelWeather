  ...
  return (
    <div className="row">
      <section className="card">
        <h2>Location</h2>
        <p className="muted" style={{marginTop:-6}}>Pick a preset or enter coordinates.</p>
        ...
        <div className="row">
          <div>
            ...
            <select
              value={coords.name}
              onChange={e => setCoords(PRESETS.find(p => p.name === e.target.value))}
            >
          ...
          <div>
            ...
            <input
              value={coords.tz}
              onChange={e => setCoords(c => ({...c, tz: e.target.value}))}
            />
          </div>
        </div>

        <div className="stack" style={{marginTop:12}}>
          <button className="btn" onClick={() => setCoords({...coords})}>
            🔄 Refresh
          </button>
          <a className="btn" href="https://www.open-meteo.com/en/docs" target="_blank" rel="noreferrer">
            📖 API Docs
          </a>
        </div>
      </section>
          ...
          <div className="row">
            ...
            <div>
              ...
              <div style={{fontSize:30, fontWeight:800}}>
                ...
                <span className="pill" title={`${data.current.wind_direction_10m}°`}>
                  {dirToText(data.current.wind_direction_10m)}
                </span>
              </div>
            </div>
          </div>
