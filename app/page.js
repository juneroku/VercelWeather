...
/* Tiny weather pictogram (sun / cloud) */
...
export default function Page() {
  ...
  const hours = useMemo(() => {
    ...
    const idx = rows.findIndex(r => new Date(r.time).getTime() >= now);
    return rows.slice(idx >= 0 ? idx : 0, (idx >= 0 ? idx : 0) + 12);
  }, [data]);
    ...
    <div style={{display:'grid', gridTemplateColumns:'340px 1fr', gap:20}}>
      ...
      <aside className="card" style={{alignSelf:'start'}}>
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8}}>
          ...
          <span className="chip" title={`${coords.name}, ${coords.country}`} style={{display:'inline-flex',alignItems:'center',gap:8}}>
            <span style={{fontSize:18}}>{flag(coords.country)}</span>
            {coords.name}
          </span>
        </div>
        ...
        <div className="row">
          <div>
            ...
            <select
              value={coords.key}
              onChange={e => setCoords(PRESETS.find(p => p.key === e.target.value) ?? PRESETS[0])}
            >
              {PRESETS.map(p => (
          ...
          {!!hours.length && (
            <div style={{overflowX:'auto', marginTop:8}}>
              <table>
                <thead>
                  <tr>
                    <th style={{position:'sticky', left:0, background:'transparent'}}>Time</th>
                    <th>Temp (°C)</th>
                    <th>Precip (mm)</th>
                    <th>Wind (km/h)</th>
                  </tr>
